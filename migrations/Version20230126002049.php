...
final class Version20230126002049 extends AbstractMigration
{
    ...
    public function up(Schema $schema): void
    {
        ...
        $departments->getColumn('company')->setLength(100);
        $departments->getColumn('contact')->setLength(100);
        $departments->getColumn('phone')->setLength(30);
        $departments->getColumn('fax')->setLength(30);
        $departments->getColumn('mobile')->setLength(30);
        $departments->getColumn('homepage')->setLength(100);
        $departments->getColumn('email')->setLength(75);
        ...
        if (!$departments->hasColumn('invoice_template_id')) {
            $departments->addColumn('invoice_template_id', 'integer', ['notnull' => false, 'default' => null]);
            $departments->addForeignKeyConstraint('kimai2_invoice_templates', ['invoice_template_id'], ['id'], ['onDelete' => 'SET NULL'], 'FK_5A97604412946D8B');
            $departments->addIndex(['invoice_template_id'], 'IDX_5A97604412946D8B');
        }

        if (!$departments->hasColumn('invoice_text')) {
            $departments->addColumn('invoice_text', 'text', ['notnull' => false, 'default' => null]);
        }
    ...
    {
        $departments = $schema->getTable('kimai2_departments');
        $departments->getColumn('company')->setLength(255);
        $departments->getColumn('contact')->setLength(255);
        $departments->getColumn('phone')->setLength(255);
        $departments->getColumn('fax')->setLength(255);
        $departments->getColumn('mobile')->setLength(255);
        $departments->getColumn('homepage')->setLength(255);

        $departments->removeForeignKey('FK_5A97604412946D8B');
        $departments->dropIndex('IDX_5A97604412946D8B');
        $departments->dropColumn('invoice_template_id');
        $departments->dropColumn('invoice_text');
