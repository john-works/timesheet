...
final class Version20200205115243 extends AbstractMigration
{
    ...
    {
        $departmentRates = $schema->createTable('kimai2_departments_rates');
        $departmentRates->addColumn('id', 'integer', ['autoincrement' => true, 'notnull' => true]);
        $departmentRates->addColumn('user_id', 'integer', ['length' => 11, 'notnull' => false]);
        $departmentRates->addColumn('department_id', 'integer', ['length' => 11, 'notnull' => false]);
        $departmentRates->addColumn('rate', 'float', ['notnull' => true]);
        $departmentRates->addColumn('fixed', 'boolean', ['notnull' => true]);
        $departmentRates->addForeignKeyConstraint('kimai2_users', ['user_id'], ['id'], ['onDelete' => 'CASCADE'], 'FK_82AB0AECA76ED395');
        $departmentRates->addForeignKeyConstraint('kimai2_departments', ['department_id'], ['id'], ['onDelete' => 'CASCADE'], 'FK_82AB0AEC9395C3F3');
        $departmentRates->addUniqueIndex(['user_id', 'department_id'], 'UNIQ_82AB0AECA76ED3959395C3F3');
        $departmentRates->setPrimaryKey(['id']);
    ...
    public function down(Schema $schema): void
    {
        ...
        $schema->dropTable('kimai2_departments_rates');
    }
}
