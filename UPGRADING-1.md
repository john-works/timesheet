...
- **BC Break**: removed `getVisible()` (deprecated since 1.4) method on Department, Project and Activity (use `isVisible()` instead, templates are still working)
...
- Rates configuration/structure changed for department, project and activity 
...
- `comments_create_department` - NEW: permission that allows to add new comments for departments  
- `comments_create_team_department` - NEW: permission that allows to add new comments for team members of the current department 
- `comments_create_teamlead_department` - NEW: permission that allows to add new comments for a teamlead of the current department 
...
- `edit_teamlead_department` - removed default permission from ROLE_TEAMLEAD (if you use it: change it in the Role & Permission UI) 
...
- `comments_department` - show comment list on department detail page (new feature) 
- `details_department` - show detail information for departments (department number, vat, rates, meta-fields, assigned teams ...)
...
If you are using teams, please read on: The following list of permissions are now also available in the UI and they (can) replace the `X_project` and `X_department` permissions. 
...
- `view_teamlead_department`, `edit_teamlead_department`, `budget_teamlead_department`, `permissions_teamlead_department`, `comments_teamlead_department`, `details_teamlead_department` - allows access to department data when user is teamlead of a team assigned to the department (replaces more global permission like `view_department` for teamleads)
- `view_team_department`, `edit_team_department`, `budget_team_department`, `comments_team_department`, `details_team_department` - allows access to department data when user is member of a team assigned to the department (replaces more global permission like `view_department` for users)
- `view_teamlead_project`, `edit_teamlead_project`, `budget_teamlead_project`, `permissions_teamlead_project`, `comments_teamlead_project`, `details_teamlead_project` - allows access to department data when user is teamlead of a team assigned to the project (replaces more global permission like `view_project` for teamleads)
- `view_team_project`, `edit_team_project`, `budget_team_project`, `comments_team_project`, `details_team_project` - allows access to department data when user is member of a team assigned to the project (replaces more global permission like `view_project` for users)
...
- Invoice changes:
  ...
  - Added new invoice fields (VAT, contact, payment details) and department field (VAT). Used the twig settings before? Move them to the respective invoice template settings.
...
- Maximum department name length lowered to 150 character
...
- `budget_department` - view and edit budgets for departments
