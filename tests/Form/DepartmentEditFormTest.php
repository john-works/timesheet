...
#[CoversClass(DepartmentEditForm::class)]
class DepartmentEditFormTest extends TypeTestCase
{
    public function testWithNewProject(): void
    {
        $model = new Department('foo');
        $form = $this->factory->createBuilder(DepartmentEditForm::class, $model);

        $attr = $form->getFormConfig()->getOption('attr');
        self::assertArrayHasKey('data-form-event', $attr);
        self::assertEquals('kimai.departmentUpdate', $attr['data-form-event']);
