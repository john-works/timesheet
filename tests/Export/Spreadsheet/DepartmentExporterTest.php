...
class DepartmentExporterTest extends TestCase
{
    public function testExport(): void
    {
        $dispatcher = $this->createMock(EventDispatcherInterface::class);
        $dispatcher->expects(self::once())->method('dispatch')->willReturnCallback(function (DepartmentMetaDisplayEvent $event) {
            $event->addField((new DepartmentMeta())->setName('foo meta')->setIsVisible(true));
            $event->addField((new DepartmentMeta())->setName('hidden meta')->setIsVisible(false));
            $event->addField((new DepartmentMeta())->setName('bar meta')->setIsVisible(true));
        ...
        $department->setCompany('Acme Foo');
        $department->setVatId('DE0123456789');
        $department->setComment('Lorem Ipsum');
        $department->setBudget(123456.7890);
        $department->setTimeBudget(1234567890);
        $department->setColor('#ababab');
        $department->setVisible(false);
        $department->setNumber('CU-0815');
        $department->setMetaField((new DepartmentMeta())->setName('foo meta')->setValue('some magic')->setIsVisible(true));
        $department->setMetaField((new DepartmentMeta())->setName('hidden meta')->setValue('will not be seen')->setIsVisible(false));
        $department->setMetaField((new DepartmentMeta())->setName('bar meta')->setValue('is happening')->setIsVisible(true));
        $department->setCountry('AT');
        $department->setAddressLine1('1234 Main Road');
        $department->setAddressLine2('Third Floor');
        $department->setAddressLine3('Golden Office Tower');
        $department->setCity('Acme City');
        $department->setPostCode('MT 543.6');
        $department->setBuyerReference('BR-0987654321');

        $sut = new EntityWithMetaFieldsExporter($spreadsheetExporter, $annotationExtractor, $metaFieldExtractor);
        $spreadsheet = $sut->export(Department::class, [$department], new DepartmentMetaDisplayEvent(new DepartmentQuery(), DepartmentMetaDisplayEvent::EXPORT));
        $worksheet = $spreadsheet->getActiveSheet();

        $i = 0;

        self::assertNull($worksheet->getCell([++$i, 2])->getValue()); // id
        self::assertEquals('test department', $worksheet->getCell([++$i, 2])->getValue()); // name
