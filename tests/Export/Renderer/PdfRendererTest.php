...
class PdfRendererTest extends AbstractRendererTestCase
{
    ...
    public function testRenderAttachment(): void
    {
        ...
        self::assertEquals('attachment; filename=' . $prefix . '-Department_Name-project_name.pdf', $response->headers->get('Content-Disposition'));
    ...
    public function testRenderInline(): void
    {
        ...
        $response = $this->render($sut);
        self::assertEquals('application/pdf', $response->headers->get('Content-Type'));
        self::assertEquals('inline; filename=' . $prefix . '-Department_Name-project_name.pdf', $response->headers->get('Content-Disposition'));
        self::assertNotEmpty($response->getContent());
    }
}
