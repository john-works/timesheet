...
class SystemConfigurationControllerTest extends AbstractControllerBaseTestCase
{
    ...
    public function getTestDataForms(): array
    {
        return [
            ...
            ['form[name=system_configuration_form_department]', $this->createUrl('/admin/system-config/update/department')],
    ...
    public function testUpdateDepartmentConfig(): void
    {
        $client = $this->getClientForAuthenticatedUser(User::ROLE_SUPER_ADMIN);
        $this->assertAccessIsGranted($client, '/admin/system-config/');

        $configService = $this->getSystemConfiguration();
        self::assertNull($configService->find('defaults.department.timezone'));
        self::assertEquals('DE', $configService->find('defaults.department.country'));
        self::assertEquals('EUR', $configService->find('defaults.department.currency'));

        $form = $client->getCrawler()->filter('form[name=system_configuration_form_department]')->form();
        $client->submit($form, [
            'system_configuration_form_department' => [
                'configuration' => [
                    ['name' => 'defaults.department.timezone', 'value' => 'Atlantic/Canary'],
                    ['name' => 'defaults.department.country', 'value' => 'BB'],
                ]
            ]
        ]);
        ...
        $configService = $this->getSystemConfiguration();
        self::assertEquals('Atlantic/Canary', $configService->find('defaults.department.timezone'));
        self::assertEquals('BB', $configService->find('defaults.department.country'));
    }

    public function testUpdateDepartmentConfigWithSingleParam(): void
    {
        $client = $this->getClientForAuthenticatedUser(User::ROLE_SUPER_ADMIN);
        $this->assertAccessIsGranted($client, '/admin/system-config/edit/department');

        $form = $client->getCrawler()->filter('form[name=system_configuration_form_department]')->form();
        self::assertStringEndsWith('/admin/system-config/update/department/1', $form->getUri());
        $client->submit($form, [
            'system_configuration_form_department' => [
                'configuration' => [
                    ['name' => 'defaults.department.timezone', 'value' => 'Atlantic/Canary'],
        ...
        $this->assertIsRedirect($client, $this->createUrl('/admin/system-config/edit/department'));
        $client->followRedirect();
        self::assertTrue($client->getResponse()->isSuccessful());
        $this->assertHasFlashSaveSuccess($client);
    }
    ...
    public function testUpdateDepartmentConfigValidation(): void
    {
        $this->assertFormHasValidationError(
            User::ROLE_SUPER_ADMIN,
            '/admin/system-config/',
            'form[name=system_configuration_form_department]',
            [
                'system_configuration_form_department' => [
                    'configuration' => [
                        ['name' => 'defaults.department.timezone', 'value' => 'XX'],
                        ['name' => 'defaults.department.country', 'value' => 1],
                    ]
                ]
            ],
            [
                '#system_configuration_form_department_configuration_0_value',
                '#system_configuration_form_department_configuration_1_value',
            ]
        );
    }
