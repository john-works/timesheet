...
class ActionsControllerTest extends APIControllerBaseTestCase
{
    ...
    public function testGetActivityActions(): void
    {
        ...
        $departments = $this->importFixture(new DepartmentFixtures(1));

        $projectFixture = new ProjectFixtures(1);
        $projectFixture->setDepartments($departments);
    ...
    public function testGetDepartmentActions(): void
    {
        $client = $this->getClientForAuthenticatedUser(User::ROLE_SUPER_ADMIN);
        ...
        foreach ($views as $view => $entries) {
            $this->assertAccessIsGranted($client, \sprintf('/api/actions/department/%s/%s/en', $departments[0]->getId(), $view));
            $result = json_decode($client->getResponse()->getContent(), true);
