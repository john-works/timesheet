...
class InvoiceServiceTest extends TestCase
{
    ...
    public function testCreateModelThrowsOnMissingTemplate(): void
    {
        ...
        $query->setDepartments([new Department('foo')]);
    ...
    public function testBeginAndEndDateFallback(): void
    {
        ...
        $query = new InvoiceQuery();
        $query->setDepartments([new Department('foo'), $department]);
        $query->setTemplate($template);
    ...
    public function testCreateModelsIncludesModelsWithNegativeTotal(): void
    {
        ...
        $department1->method('getId')->willReturn(1);
        $department1->method('getName')->willReturn('department1');
        $department1->method('isVisible')->willReturn(true);
        $project1 = new Project();
        $project1->setDepartment($department1);

        $department2 = $this->createMock(Department::class);
        $department2->method('getId')->willReturn(2);
        $department2->method('getName')->willReturn('department2');
        $department2->method('isVisible')->willReturn(true);
        $project2 = new Project();
        $project2->setDepartment($department2);

        $department3 = $this->createMock(Department::class);
        $department3->method('getId')->willReturn(3);
        $department3->method('getName')->willReturn('department3');
        $department3->method('isVisible')->willReturn(true);
        $project3 = new Project();
        $project3->setDepartment($department3);

        $department4 = $this->createMock(Department::class);
        $department4->method('getId')->willReturn(4);
        $department4->method('getName')->willReturn('department4');
        $department4->method('isVisible')->willReturn(true);
        $project4 = new Project();
        $project4->setDepartment($department4);
        ...
        $query = new InvoiceQuery();
        $query->setDepartments([$department3, new Department('tröööt'), $department1, $department3]);
        $query->setTemplate($template);
        self::assertNull($query->getBegin());
        self::assertNull($query->getEnd());
