...
class ConfigurableNumberGeneratorTest extends TestCase
{
    private function getSut(string $format, int $counter = 1): ConfigurableNumberGenerator
    {
        ...
        $repository
            ->expects($this->any())
            ->method('getCounterForDepartmentAllTime')
            ->willReturn($counter);
        $repository
            ->expects($this->any())
    ...
    {
        $department = new Department('Acme company');
        $department->setNumber('0815');
        ...
        $model = (new InvoiceModelFactoryFactory($this))->create()->createModel(new DebugFormatter(), $department, new InvoiceTemplate(), new InvoiceQuery());
    ...
    public function testInvalidGetInvoiceNumber(string $format, \DateTime $invoiceDate, string $brokenPart): void
    {
        ...
        $model = (new InvoiceModelFactoryFactory($this))->create()->createModel(new DebugFormatter(), new Department('foo'), new InvoiceTemplate(), new InvoiceQuery());
        $model->setInvoiceDate($invoiceDate);
        $sut->setModel($model);
    ...
    {
        return [
            ['{Y}/{cnumber}_{ccy,3}', 'Department has no number, replacer {cnumber} failed evaluation'],
            ['{Y}/{cname}_{ccy,3}', 'Department has no name, replacer {cname} failed evaluation'],
        ];
    }

    #[DataProvider('getMissingFieldTestData')]
    public function testDepartmentHasMissingField(string $format, string $message): void
    {
        $this->expectException(\InvalidArgumentException::class);
