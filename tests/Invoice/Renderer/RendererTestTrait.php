...
trait RendererTestTrait
{
    ...
    protected function getInvoiceModel(): InvoiceModel
    {
        ...
        $department = new Department('department,with/special#name');
        $department->setAddress('Foo' . PHP_EOL . 'Street' . PHP_EOL . '1111 City');
        $department->setCurrency('EUR');
        $department->setCountry('AT');
        $department->setMetaField((new DepartmentMeta())->setName('foo-department')->setValue('bar-department')->setIsVisible(true));
        ...
        $model = (new InvoiceModelFactoryFactory($this))->create()->createModel($this->getFormatter(), $department, $template, $query);
