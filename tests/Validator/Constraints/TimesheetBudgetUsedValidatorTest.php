...
class TimesheetBudgetUsedValidatorTest extends ConstraintValidatorTestCase
{
     ...
     */
    protected function createValidator(bool $isAllowed = false, ?ActivityBudgetStatisticModel $activityStatisticModel = null, ?ProjectBudgetStatisticModel $projectStatisticModel = null, ?DepartmentBudgetStatisticModel $departmentStatisticModel = null, ?array $rawData = null, ?Rate $rate = null): TimesheetBudgetUsedValidator
    {
        $configuration = SystemConfigurationFactory::createStub(['timesheet' => ['rules' => ['allow_overbooking_budget' => $isAllowed]]]);

        if ($departmentStatisticModel === null) {
            $departmentStatisticModel = new DepartmentBudgetStatisticModel(new Department('foo'));
            $departmentStatistic = new DepartmentStatistic();
            $departmentStatisticModel->setStatisticTotal($departmentStatistic);
            $departmentStatisticModel->setStatistic($departmentStatistic);
        }

        $departmentRepository = $this->createMock(DepartmentStatisticService::class);
        $departmentRepository->method('getBudgetStatisticModel')->willReturn($departmentStatisticModel);
        ...
        return new TimesheetBudgetUsedValidator($configuration, $departmentRepository, $projectRepository, $activityRepository, $timesheetRepository, $rateService, $auth, $localeService);
    }
    ...
    public static function getViolationTestData(): array
    {
        return [
            ...
            'c_v' => [null, null, null, null, 1230, null,       null, null, null, null, null, null, null, 3600, null,       '0:20', '0:39', '1:00', 'department',          '+3600 seconds'],
            'c_w' => [null, null, null, null, null, 1001.0,     null, null, null, null, null, null, null, null, 1000.0,     '€1,001.00', '€0.00', '€1,000.00', 'department',  '+3600 seconds'],

            //        previously logged                         available budgets                                           expected violation                              duration            entry currently in database
            'c_f1' => [null, null, null, null, 1320, null,      null, null, null, null, null, null, null, 3600, null,       '0:22', '0:38', '1:00', 'department',          '+3600 seconds',    ['rate' => 1.0, 'duration' => 1000]],
            'c_h1' => [null, null, null, null, 7200, null,      null, null, null, null, null, null, null, 7200, null,       '2:00', '0:00', '2:00', 'department',          '+3601 seconds',    ['rate' => 1.0, 'duration' => 3600]],
            ...
            'c_g0' => [null, null, null, null, null, 1002.0,    null, null, null, null, null, null, null, null, 1000.0,     '€1,002.00', '€0.00', '€1,000.00', 'department',     '+3600 seconds',    ['rate' => 1.0, 'duration' => 1010]],
    ...
    public function testWithActivityTimeBudget(
        ...
        ?int $projectDuration,
        ?float $projectRate,
        ?int $departmentDuration,
        ?float $departmentRate,
        ...
        ?float $projectBudget,
        ?string $departmentBudgetType,
        ?int $departmentTimeBudget,
        ...
        $departmentStatistic = new DepartmentStatistic();
        if ($departmentDuration !== null) {
            $departmentStatistic->setDuration($departmentDuration);
            $departmentStatistic->setDurationBillable($departmentDuration);
        }
        if ($departmentRate !== null) {
            $departmentStatistic->setRate($departmentRate);
            $departmentStatistic->setRateBillable($departmentRate);
        }
        ...
        if (!empty($rawData)) {
            ...
            if (!\array_key_exists('department', $rawData)) {
                $rawData['department'] = 1;
            }
            ...
            $department->method('getCurrency')->willReturn('EUR');
            $department->method('getId')->willReturn($rawData['department']);
            $department->method('isMonthlyBudget')->willReturn(false);
            if ($departmentBudgetType !== null) {
                $department->method('getBudgetType')->willReturn($departmentBudgetType);
                $department->method('isMonthlyBudget')->willReturn($departmentBudgetType === 'month');
            }
            if ($departmentTimeBudget !== null) {
                $department->method('getTimeBudget')->willReturn($departmentTimeBudget);
                $department->method('hasTimeBudget')->willReturn(true);
                $department->method('hasBudgets')->willReturn(true);
            }
            if ($departmentBudget !== null) {
                $department->method('getBudget')->willReturn($departmentBudget);
                $department->method('hasBudget')->willReturn(true);
                $department->method('hasBudgets')->willReturn(true);
            }

            $project = $this->createMock(Project::class);
            $project->method('getId')->willReturn($rawData['project']);
            $project->method('getDepartment')->willReturn($department);
            ...
            if ($departmentTimeBudget !== null) {
                $department->setTimeBudget($departmentTimeBudget);
            }
            if ($departmentBudget !== null) {
        ...
        $departmentBudgetStatisticModel = new DepartmentBudgetStatisticModel($department);
        $departmentBudgetStatisticModel->setStatistic($departmentStatistic);
        $departmentBudgetStatisticModel->setStatisticTotal($departmentStatistic);

        $this->validator = $this->createValidator(false, $activityBudgetStatisticModel, $projectBudgetStatisticModel, $departmentBudgetStatisticModel, $rawData, $rate);
        $this->validator->initialize($this->context);
