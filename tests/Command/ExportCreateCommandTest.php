...
class ExportCreateCommandTest extends KernelTestCase
{
    ...
    {
        $options = ['--set-exported' => null, '--department' => [1], '--template' => 'csv', '--start' => '2020-01-01', '--end' => '2020-03-01'];
        $commandTester = $this->createExport($options);
     ...
     * @return array{0: Department, 1: array<Project>}
     */
    ...
    public function testCreateExportByDepartment(): void
    {
        ...
        $commandTester = $this->createExport(['--template' => 'csv', '--department' => [$data[0]->getId()], '--start' => $start->format('Y-m-d'), '--end' => $end->format('Y-m-d'), '--username' => UserFixtures::USERNAME_SUPER_ADMIN]);
