...
class InvoiceVoterTest extends AbstractVoterTestCase
{
    ...
    public function testVoteDeniesIfInvoiceHasNoDepartment(): void
    {
        $this->assertVote(self::getUser(2, User::ROLE_TEAMLEAD), new Invoice(), 'view_invoice', VoterInterface::ACCESS_DENIED);
    }

    public function testVoteRequiresDepartmentTeamAccess(): void
    {
        $department = new Department('Acme');
        $department->addTeam(new Team('Accounting'));
        $invoice = $this->createInvoice($department);
    ...
    public function testDeleteInvoiceRequiresDeletePermission(): void
    {
        ...
        self::assertEquals(VoterInterface::ACCESS_GRANTED, $sut->vote($token, $this->createInvoice($department), ['delete_invoice']));
    }
    ...
    private function createInvoice(?Department $department = null): Invoice
    {
        $invoice = new Invoice();
        if ($department !== null) {
    ...
    {
        $invoice = new Invoice();
        $invoice->setDepartment(new Department('Acme'));
