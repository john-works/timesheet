...
class TimesheetVoterTest extends AbstractVoterTestCase
{
    ...
    public function testSpecialCases(): void
    {
        ...
        // hidden departments might not be started
        $timesheet = self::getTimesheet($user1);
        $timesheet->getProject()?->getDepartment()?->setVisible(false);
        $this->assertVote($user2, $timesheet, 'start', VoterInterface::ACCESS_DENIED);
    ...
    public function testOwnerCanAccessOwnTimesheetEvenWithRestrictiveTeams(): void
    {
        ...
        $department->addTeam(new Team('locked department team'));
    ...
    public function testTeamleadDeniedWhenDepartmentTeamBlocks(): void
    {
        $owner = self::getUser(1, User::ROLE_USER);
        $requester = self::getUser(2, User::ROLE_TEAMLEAD);

        // Even though requester is a teamlead of the owner's team, the department
        // team blocks access.
        $sharedTeam = new Team('shared');
        $sharedTeam->addUser($owner);
        $sharedTeam->addTeamlead($requester);
    ...
    public function testAdminWithoutCanSeeAllDataIsBlockedByDepartmentTeam(): void
    {
    ...
    {
        // Confirms that the department/project/activity gate uses the simpler
        ...
        $department = new Department('Acme');
        $department->addTeam($departmentTeam);
        $departmentTeam->addUser($requester); // plain member is enough here
    ...
    private static function getTimesheetFor(User $owner, ?Team $departmentTeam = null, ?Team $projectTeam = null, ?Team $activityTeam = null): Timesheet
    {
        $department = new Department('Acme');
