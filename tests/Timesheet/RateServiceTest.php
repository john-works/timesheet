...
class RateServiceTest extends TestCase
{
    ...
    {   //             expected, expInt, durat, userH,  userIn, timeH,  timeF,  actH,   actIn,  actF,    proH,   proIn,  proFi,   custH,  custIn, custF
        ...
        // rate: 2.0 => timesheet fixed rate , internal: 5.0 => department hourly rate
    ...
    public function testRates(
        ...
        ?float $departmentInternal,
        bool $departmentIsFixed
    ): void {
        ...
        if (null !== $departmentRate) {
            $rate = new DepartmentRate();
            $rate->setRate($departmentRate);
            $rate->setIsFixed($departmentIsFixed);
            if (null !== $departmentInternal) {
                $rate->setInternalRate($departmentInternal);
            }
            $rates[] = $rate;
        }
