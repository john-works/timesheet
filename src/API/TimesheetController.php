...
final class TimesheetController extends BaseApiController
{
    ...
    #[Rest\QueryParam(name: 'department', requirements: '\d+', strict: true, nullable: true, description: 'Department ID to filter timesheets')]
    #[Rest\QueryParam(name: 'departments', map: true, requirements: '\d+', strict: true, nullable: true, default: [], description: 'List of department IDs to filter, e.g.: departments[]=1&departments[]=2')]
    ...
    public function cgetAction(ParamFetcherInterface $paramFetcher, DepartmentRepository $departmentRepository, ProjectRepository $projectRepository, ActivityRepository $activityRepository, UserRepository $userRepository): Response
    {
        ...
        $departments = $paramFetcher->get('departments');
        $cu = $paramFetcher->get('department');
        if (\is_string($cu) && $cu !== '') {
            $departments[] = $cu;
        }

        foreach ($departmentRepository->findByIds(array_unique($departments)) as $department) {
            if (!$this->isGranted('access', $department)) {
                throw $this->createAccessDeniedException('Cannot access Department: ' . $department->getId());
            }
    ...
    #[OA\Response(response: 200, description: 'Returns a collection of recent user activities (always the latest entry of a unique working set grouped by department, project and activity)', content: new OA\JsonContent(type: 'array', items: new OA\Items(ref: '#/components/schemas/TimesheetCollectionExpanded')))]
