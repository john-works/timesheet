...
final class TeamController extends BaseApiController
{
     ...
     *
     * The team is granted access to the department.
     */
    #[IsGranted('edit', 'team')]
    #[OA\Post(responses: [new OA\Response(response: 200, description: 'Returns the team including the department', content: new OA\JsonContent(ref: '#/components/schemas/Team'))])]
    #[OA\Parameter(name: 'id', in: 'path', description: 'The team that is granted access', required: true)]
    #[OA\Parameter(name: 'departmentId', in: 'path', description: 'The department to grant acecess to (Department ID)', required: true)]
    #[Route(methods: ['POST'], path: '/{id}/departments/{departmentId}', name: 'post_team_department', requirements: ['id' => '\d+', 'departmentId' => '\d+'])]
    public function postDepartmentAction(Team $team, #[MapEntity(mapping: ['departmentId' => 'id'])] Department $department, DepartmentRepository $departmentRepository): Response
    {
        if ($team->hasDepartment($department)) {
            throw new BadRequestHttpException('Team has already access to department');
        }
     ...
     *
     * This removes access to the department from the team.
     */
    #[IsGranted('edit', 'team')]
    #[OA\Delete(responses: [new OA\Response(response: 200, description: 'Returns the team without the department', content: new OA\JsonContent(ref: '#/components/schemas/Team'))])]
    #[OA\Parameter(name: 'id', in: 'path', description: 'The team whose permission will be revoked', required: true)]
    #[OA\Parameter(name: 'departmentId', in: 'path', description: 'The department to remove (Department ID)', required: true)]
    #[Route(methods: ['DELETE'], path: '/{id}/departments/{departmentId}', name: 'delete_team_department', requirements: ['id' => '\d+', 'departmentId' => '\d+'])]
    public function deleteDepartmentAction(Team $team, #[MapEntity(mapping: ['departmentId' => 'id'])] Department $department, DepartmentRepository $departmentRepository): Response
    {
        if (!$team->hasDepartment($department)) {
            throw new BadRequestHttpException('Department is not assigned to the team');
        }
