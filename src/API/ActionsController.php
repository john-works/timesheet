...
final class ActionsController extends BaseApiController
{
    ...
    /**
     * Fetch item actions for Department
     */
    #[OA\Response(response: 200, description: 'Returns item actions for the department', content: new OA\JsonContent(ref: new Model(type: PageAction::class)))]
    #[OA\Parameter(name: 'id', in: 'path', description: 'Department ID to fetch', required: true)]
    #[OA\Parameter(name: 'view', in: 'path', description: 'View to display the actions at (e.g. index, custom)', required: true)]
    #[OA\Parameter(name: 'locale', in: 'path', description: 'Language to translate the action title to (e.g. de, en)', required: true)]
    #[OA\Get(x: ['internal' => true])]
    #[Route(methods: ['GET'], path: '/department/{id}/{view}/{locale}', name: 'get_department_actions', requirements: ['id' => '\d+'])]
    #[IsGranted('view', 'department')]
    public function getDepartmentActions(Department $department, string $view, string $locale): Response
    {
        $event = new PageActionsEvent($this->getUser(), ['department' => $department], 'department', $view);
        $actions = $this->convertEvent($event, $locale);
