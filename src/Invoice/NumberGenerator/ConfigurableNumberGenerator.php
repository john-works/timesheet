...
final class ConfigurableNumberGenerator implements NumberGeneratorInterface
{
    ...
    public function getInvoiceNumber(): string
    {
        ...
        $numberGenerator = new NumberGenerator($format, function (string $originalFormat, string $format, int $increaseBy) use ($invoiceDate): string|int {
            ...
            if ($format === 'cname' && $this->model->getDepartment()->getName() === null) {
                throw new \InvalidArgumentException('Department has no name, replacer {cname} failed evaluation');
            }

            if ($format === 'cnumber' && $this->model->getDepartment()->getNumber() === null) {
                throw new \InvalidArgumentException('Department has no number, replacer {cnumber} failed evaluation');
            }
            ...
            return match ($format) {
                ...
                'cc' => $this->repository->getCounterForDepartmentAllTime($this->model->getDepartment()) + $increaseBy,
                'ccy' => $this->repository->getCounterForYear($invoiceDate, $this->model->getDepartment()) + $increaseBy,
                'ccm' => $this->repository->getCounterForMonth($invoiceDate, $this->model->getDepartment()) + $increaseBy,
                'ccd' => $this->repository->getCounterForDay($invoiceDate, $this->model->getDepartment()) + $increaseBy,
                ...
                'c' => $this->repository->getCounterForDepartmentAllTime() + $increaseBy,
                ...
                'cname' => (string) $this->model->getDepartment()->getName(),
                'cnumber' => (string) $this->model->getDepartment()->getNumber(),
