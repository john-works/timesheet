...
class InvoiceService
{
    ...
    public function createInvoice(InvoiceModel $model, EventDispatcherInterface $dispatcher): Invoice
    {
        ...
        foreach ($this->getRenderer() as $renderer) {
            if ($renderer->supports($document)) {
                ...
                if (!$invoice->getDepartment()->hasInvoiceTemplate()) {
                    $invoice->getDepartment()->setInvoiceTemplate($model->getTemplate());
                }
                $this->saveInvoice($invoice);
    ...
    private function createModelWithoutEntries(InvoiceQuery $query): InvoiceModel
    {
        ...
        if ($department === null) {
            throw new \Exception('Cannot create invoice model without department');
        }

        $template = $query->getTemplate();

        if ($query->isAllowTemplateOverwrite() && $department->hasInvoiceTemplate()) {
            $template = $department->getInvoiceTemplate();
        }
    ...
    public function createModels(InvoiceQuery $query): array
    {
        ...
        foreach ($items as $entry) {
            $department = $entry->getProject()->getDepartment();
            if ($department === null || !$department->isVisible()) { // generating invoices for hidden departments does not yet work
                continue;
            }
            $id = $department->getId();
            if (!\array_key_exists($id, $departmentEntries)) {
                $departmentEntries[$id] = [
                    'department' => $department,
                    'entries' => [],
                ];
            }
            $departmentEntries[$id]['entries'][] = $entry;
        }

        if (empty($departmentEntries)) {
            return [];
        }

        uasort($departmentEntries, function ($a, $b): int {
            $nameA = $a['department']->getName();
            $nameB = $b['department']->getName();
        ...
        foreach ($departmentEntries as $settings) {
            $departmentQuery = clone $query;
            $departmentQuery->setDepartments([$settings['department']]);
            $model = $this->createModelWithoutEntries($departmentQuery);
