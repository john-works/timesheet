...
final class InvoiceCreateCommand extends Command
{
    ...
    protected function configure(): void
    {
        $this
            ...
            ->addOption('department', null, InputOption::VALUE_OPTIONAL, 'Comma separated list of department IDs', null)
            ->addOption('project', null, InputOption::VALUE_OPTIONAL, 'Comma separated list of project IDs', null)
            ->addOption('by-department', null, InputOption::VALUE_NONE, 'If set, one invoice for each active department in the given timerange is created')
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $byActiveDepartment = $input->getOption('by-department');
        $byActiveProject = $input->getOption('by-project');

        if ($byActiveDepartment && $byActiveProject) {
            $io->error('You cannot mix "by-department" and "by-project"');

            return Command::FAILURE;
        }

        $departmentsIDs = $input->getOption('department');
        $projectIDs = $input->getOption('project');
        if (!$byActiveDepartment && !$byActiveProject && empty($departmentsIDs) && empty($projectIDs)) {
            $io->error('Could not determine generation mode, you need to set one of: department, project, by-department, by-project');
        ...
        if (!empty($departmentsIDs)) {
            ...
            $departmentsIDs = explode(',', $departmentsIDs);
            foreach ($departmentsIDs as $id) {
                $tmp = $this->departmentRepository->find($id);
                if (null === $tmp) {
                    $io->error('Unknown department ID: ' . $id);

                    return Command::FAILURE;
                }
                $departments[] = $tmp;
            }
            $invoices = $this->createInvoicesForDepartment($departments, $defaultQuery, $input, $output);
        ...
        } elseif ($byActiveDepartment) {
            $departments = $this->getActiveDepartments($defaultQuery);
            $invoices = $this->createInvoicesForDepartment($departments, $defaultQuery, $input, $output);
    ...
    protected function createInvoicesForProjects(array $projects, InvoiceQuery $defaultQuery, InputInterface $input, OutputInterface $output): array
    {
        ...
        foreach ($projects as $project) {
            ...
            if ($department === null) {
                throw new \Exception('Project has no department: ' . $project->getId());
            }
            ...
            $tpl = $this->getTemplateForDepartment($input, $department);
            if (null === $tpl) {
     ...
     * @throws \Exception
     */
    protected function createInvoicesForDepartment(array $departments, InvoiceQuery $defaultQuery, InputInterface $input, OutputInterface $output): array
    {
        $io = new SymfonyStyle($input, $output);
        ...
        foreach ($departments as $department) {
            ...
            $tpl = $this->getTemplateForDepartment($input, $department);
            if (null === $tpl) {
                $io->warning('Could not find invoice template for department, skipping.');
                continue;
            }
            $query->setTemplate($tpl);
            ...
            try {
                ...
                $io->error(\sprintf('Failed to create invoice for department with: %s', $ex->getMessage()));
            }
        }
    ...
    protected function renderInvoiceResult(InputInterface $input, OutputInterface $output, array $invoices): int
    {
        ...
        $columns = ['ID', 'Department', 'Total', 'Filename'];
        ...
        foreach ($invoices as $invoice) {
            ...
            $table->addRow([
                $invoice->getId(),
                $invoice->getDepartment()->getName(),
                $invoice->getTotal() . ' ' . $invoice->getDepartment()->getCurrency(),
                $file->getRealPath()
            ]);
        }
    ...
    private function getTemplateForDepartment(InputInterface $input, Department $department): ?InvoiceTemplate
    {
        $template = $input->getOption('template');

        if (null === $template) {
            return $department->getInvoiceTemplate();
        }
     ...
     * @return Department[]
     */
    private function getActiveDepartments(InvoiceQuery $invoiceQuery): array
    {
        ...
        foreach ($results as $result) {
            $department = $result->getProject()->getDepartment();
