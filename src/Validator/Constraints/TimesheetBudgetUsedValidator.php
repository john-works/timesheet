...
final class TimesheetBudgetUsedValidator extends ConstraintValidator
{
    public function __construct(
        ...
        private readonly DepartmentStatisticService $departmentStatisticService,
    ...
    public function validate(mixed $value, Constraint $constraint): void
    {
        ...
        if ($id !== null) {
            ...
            $departmentId = (int) $rawData['department'];
            ...
            if ($rawData['billable']) {
                ...
                if (null !== $value->getProject()) {
                    ...
                    if ($departmentId === $value->getProject()->getDepartment()->getId()) {
                        $departmentDuration -= $rawData['duration'];
                        $departmentRate -= $rawData['rate'];
                    }
                }
            }
        ...
        if (null !== ($project = $value->getProject())) {
            ...
            if (null !== ($department = $project->getDepartment()) && $department->hasBudgets()) {
                $dateTime = $department->isMonthlyBudget() ? $recordDate : $now;
                if ($department->isMonthlyBudget() && $monthWasChanged) {
                    $departmentDuration = $duration;
                }
                $stat = $this->departmentStatisticService->getBudgetStatisticModel($department, $dateTime);
                $this->checkBudgets($constraint, $stat, $value, $departmentDuration, $departmentRate, 'department');
            }
        }
    }
    ...
    private function addBudgetViolation(TimesheetBudgetUsed $constraint, TimesheetEntity $timesheet, string $field, float $budget, float $rate): void
    {
        ...
        $currency = $timesheet->getProject()->getDepartment()->getCurrency();
