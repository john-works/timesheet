...
final class Configuration implements ConfigurationInterface
{
        ...
        $node
            ...
            ->children()
                ...
                ->enumNode('title_pattern')
                    ->values(['{activity}', '{project}', '{department}', '{description}'])
    ...
    private function getDepartmentNode(): ArrayNodeDefinition
    {
        $builder = new TreeBuilder('department');
        /** @var ArrayNodeDefinition $node */
        $node = $builder->getRootNode();
        ...
        $node
            ...
            ->children()
                ->scalarNode('choice_pattern')
                    ->defaultValue(DepartmentHelper::PATTERN_NAME)
                ->end()
                ->scalarNode('number_format')
                    ->defaultValue('{cc,4}')
                ->end()
    ...
    private function getDefaultsNode(): ArrayNodeDefinition
    {
        ...
        $node
            ->addDefaultsIfNotSet()
            ->children()
                ->arrayNode('department')
                    ->addDefaultsIfNotSet()
                    ->children()
                        ->scalarNode('timezone')->defaultNull()->end()
                        ->scalarNode('country')->defaultValue('DE')->end()
                        ->scalarNode('currency')->defaultValue(Department::DEFAULT_CURRENCY)->end()
                    ->end()
                ->end()
                ->arrayNode('user')
                    ->addDefaultsIfNotSet()
