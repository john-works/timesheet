...
class TimesheetEditForm extends AbstractType
{
    ...
    public function buildForm(FormBuilderInterface $builder, array $options): void
    {
        ...
        // -----------------------------------------------------
        $query = new DepartmentFormTypeQuery($department);
        $query->setUser($options['user']); // @phpstan-ignore-line
        $qb = $this->departments->getQueryBuilderForFormType($query);
        /** @var array<Department> $departments */
        $departments = $qb->getQuery()->getResult();
        $departmentCount = \count($departments);

        if ($this->showDepartment($options, $isNew, $departmentCount)) {
    ...
    protected function showDepartment(array $options, bool $isNew, int $departmentCount): bool
    {
        if (!$isNew && $options['department']) {
            return true;
        }
    ...
    public function configureOptions(OptionsResolver $resolver): void
    {
        ...
        $resolver->setDefaults([
            ...
            'department' => false, // for API usage
