...
final class DepartmentHelper
{
    ...
    public function getChoicePattern(): string
    {
        if ($this->pattern === null) {
            $this->pattern = $this->configuration->find('department.choice_pattern');
    ...
    public function getChoiceLabel(Department $department): string
    {
        ...
        $name = str_replace(self::PATTERN_NAME, $department->getName() ?? '', $name);
        $name = str_replace(self::PATTERN_COMMENT, $department->getComment() ?? '', $name);
        $name = str_replace(self::PATTERN_NUMBER, $department->getNumber() ?? '', $name);
        $name = str_replace(self::PATTERN_COMPANY, $department->getCompany() ?? '', $name);
