...
final class ProjectSubscriber extends AbstractActionsSubscriber
{
    ...
    public function onActions(PageActionsEvent $event): void
    {
        ...
        if ($project->getId() === null || $department === null) {
            return;
        }
        ...
        if ($this->isGranted('view_activity')) {
            $event->addActionToSubmenu('filter', 'activity', ['title' => 'activities', 'url' => $this->path('admin_activity', ['departments[]' => $department->getId(), 'projects[]' => $project->getId()])]);
        }

        if ($this->isGranted('view_other_timesheet')) {
            $event->addActionToSubmenu('filter', 'timesheet', ['title' => 'timesheet.filter', 'url' => $this->path('admin_timesheet', ['departments[]' => $department->getId(), 'projects[]' => $project->getId(), 'daterange' => $dateRange])]);
        }

        if ($this->isGranted('create_export')) {
            $event->addActionToSubmenu('filter', 'export', ['title' => 'export', 'url' => $this->path('export', ['departments[]' => $department->getId(), 'projects[]' => $project->getId(), 'exported' => 1, 'daterange' => $dateRange])]);
        }
        ...
        if ($isListingView) {
            if ($project->isVisible() && $department->isVisible() && $this->isGranted('create_activity')) {
        ...
        if (($event->isIndexView() || $event->isView('department_details')) && $this->isGranted('delete', $project)) {
