        ...
        if ($form->isValid() && ($query->hasBookmark() || $request->query->has('performSearch'))) {
            try {
                ...
                foreach ($entries as $entry) {
                    $cid = $entry->getProject()->getDepartment()->getId();
                    if (!isset($byDepartment[$cid])) {
                        $byDepartment[$cid] = [
                    ...
                    $byDepartment[$cid]['rate'] += $entry->getRate();
                    $byDepartment[$cid]['internalRate'] += $entry->getInternalRate() ?? 0.0;
                    $byDepartment[$cid]['duration'] += $entry->getDuration() ?? 0;
                }
