...
abstract class AbstractUserReportController extends AbstractController
{
    ...
    protected function prepareReport(DateTimeInterface $begin, DateTimeInterface $end, User $user): array
    {
        ...
        foreach ($data as $id => $row) {
            $departmentId = (string) $row['project']->getDepartment()->getId();
            if (!\array_key_exists($departmentId, $departments)) {
                $departments[$departmentId] = [
                    'department' => $row['project']->getDepartment(),
                    'projects' => [],
                    'duration' => 0,
                    'rate' => 0.0,
                    'internalRate' => 0.0,
            ...
            $departments[$departmentId]['projects'][$id] = $row;
            $departments[$departmentId]['duration'] += $row['duration'];
            $departments[$departmentId]['rate'] += $row['rate'];
            $departments[$departmentId]['internalRate'] += $row['internalRate'];
        }
