...
final class InvoiceController extends AbstractController
{
    ...
    public function indexAction(Request $request, InvoiceService $service, InvoiceTemplateRepository $templateRepository): Response
    {
        ...
        foreach ($models as $model) {
            ...
            $departmentTpl = $model->getTemplate();
            $total += \count($model->getCalculator()->getEntries());

            $values = [
                'invoiceDate' => null,
                'template' => $departmentTpl
            ];

            $forms[] = $this->createFormWithName('department_' . $department->getId(), FormType::class, $values, [
    ...
    #[Route(path: '/preview/{department}/{token}', name: 'invoice_preview', methods: ['GET'])]
    #[IsGranted('create_invoice')]
    #[IsGranted('access', 'department')]
    public function previewAction(Department $department, string $token, Request $request, InvoiceService $service): Response
    {
        ...
        if ($form->isValid()) {
            try {
                $query->setDepartments([$department]);
    ...
    #[Route(path: '/save-invoice/{department}/{token}', name: 'invoice_create', methods: ['GET'])]
    #[IsGranted('create_invoice')]
    #[IsGranted('access', 'department')]
    public function createInvoiceAction(Department $department, string $token, Request $request, DepartmentRepository $departmentRepository, InvoiceService $service): Response
    {
        ...
        if ($form->isValid()) {
            try {
                ...
                // save default template for department if not yet set
                if ($department->getInvoiceTemplate() === null) {
                    $department->setInvoiceTemplate($query->getTemplate());
    ...
    public function showInvoicesAction(Request $request, int $page, InvoiceRepository $invoiceRepository): Response
    {
        ...
        $table->addColumn('department', ['class' => 'alwaysVisible text-nowrap', 'orderBy' => false]);
