...
use App\Form\Type\DepartmentTypePatternType;
...
final class SystemConfigurationController extends AbstractController
{
    ...
    private function getConfigurationTypes(): array
    {
        ...
        return [
            ...
            $authentication,
            (new SystemConfigurationModel('department'))
                ->setConfiguration([
                    (new Configuration('defaults.department.timezone'))
                        ->setLabel('timezone')
                        ->setType(TimezoneType::class)
                        ->setValue(date_default_timezone_get())
                        ->setOptions(['help' => 'default_value_new']),
                    (new Configuration('defaults.department.country'))
                        ->setLabel('country')
                        ->setType(CountryType::class)
                        ->setOptions(['help' => 'default_value_new']),
                    (new Configuration('department.choice_pattern'))
                        ->setLabel('choice_pattern')
                        ->setType(DepartmentTypePatternType::class),
                    (new Configuration('department.number_format'))
                        ->setLabel('department.number_format')
                        ->setOptions(['help' => 'allowed_replacer', 'help_translation_parameters' => ['%replacer%' => '{cc}, {Y}, {y}, {M}, {m}, {D}, {d}, {YY}, {yy}, {MM}, {DD}']])
                        ->setRequired(true)
                        ->setType(TextType::class)
                        ->setTranslationDomain('system-configuration'),
                    (new Configuration('department.rules.allow_duplicate_number'))
            ...
            (new SystemConfigurationModel('branding'))
                ->setConfiguration([
                    ...
                    (new Configuration('theme.branding.company'))
                        ...
                        ->setRequired(false)
                        ->setType(TextType::class),
                    (new Configuration('defaults.department.currency'))
                        ->setLabel('currency')
                        ->setType(CurrencyType::class)
                        ->setOptions(['help' => 'Can be overwritten per department']),
