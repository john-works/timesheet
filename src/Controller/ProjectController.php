...
final class ProjectController extends AbstractController
{
    ...
    public function indexAction(int $page, Request $request, EventDispatcherInterface $dispatcher): Response
    {
        ...
        $table->addColumn('department', ['class' => 'd-none']);
    ...
    #[Route(path: '/create/{department}', name: 'admin_project_create_with_department', methods: ['GET', 'POST'])]
    #[IsGranted('create_project')]
    public function createWithDepartmentAction(Request $request, Department $department, ProjectService $projectService, SystemConfiguration $configuration): Response
    {
        return $this->createProject($request, $projectService, $configuration, $department);
    }
    ...
    private function createProject(Request $request, ProjectService $projectService, SystemConfiguration $configuration, ?Department $department = null): Response
    {
        $project = $projectService->createNewProject($department);
    ...
    public function detailsAction(Project $project, TeamRepository $teamRepository, ProjectRateRepository $rateRepository, ProjectStatisticService $statisticService, ProjectService $projectService, CsrfTokenManagerInterface $csrfTokenManager, EventDispatcherInterface $dispatcher): Response
    {
        ...
        $exportUrl = null;
        $invoiceUrl = null;
        if ($this->isGranted('create_export') && $project->getDepartment() !== null) {
            $exportUrl = $this->generateUrl('export', ['departments[]' => $project->getDepartment()->getId(), 'projects[]' => $project->getId(), 'daterange' => '', 'exported' => TimesheetQuery::STATE_NOT_EXPORTED, 'preview' => true, 'billable' => true]);
        }
        if ($this->isGranted('view_invoice') && $project->getDepartment() !== null) {
            $invoiceUrl = $this->generateUrl('invoice', ['departments[]' => $project->getDepartment()->getId(), 'projects[]' => $project->getId(), 'daterange' => '', 'exported' => TimesheetQuery::STATE_NOT_EXPORTED, 'billable' => true]);
        }
    ...
    private function createEditForm(Project $project, string $currency): FormInterface
    {
        ...
        if ($project->getId() !== null) {
            ...
            $currency = $project->getDepartment()->getCurrency();
        }
