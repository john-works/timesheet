...
final class TimesheetLoader implements LoaderInterface
{
    ...
    public function loadResults(array $results): void
    {
        ...
        if (\count($projectIds) > 0) {
            ...
            $projects = $qb->select('PARTIAL p.{id}', 'department')
                ->from(Project::class, 'p')
                ->leftJoin('p.department', 'department')
                ->andWhere($qb->expr()->in('p.id', $projectIds))
                ->getQuery()
                ->execute();

            if ($this->query !== null && $this->query->hasQueryHint(TimesheetQueryHint::DEPARTMENT_META_FIELDS)) {
