...
class TeamRepository extends EntityRepository
{
    ...
    private function getQueryBuilderForQuery(TeamQuery $query): QueryBuilder
    {
        ...
        if ($query->hasDepartments()) {
            $qb->leftJoin('t.departments', 'qDepartments');
            $qb->orWhere(
                $qb->expr()->in('qDepartments', ':departments')
            );
            $qb->setParameter('departments', $query->getDepartments());
        }
    ...
    public function prepareTeamQuery(Query $query): Query
    {
        ...
        // $query->setFetchMode(Team::class, 'departments', ClassMetadata::FETCH_EAGER);
