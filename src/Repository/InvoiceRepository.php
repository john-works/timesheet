...
class InvoiceRepository extends EntityRepository
{
    ...
    private function getCounterFor(\DateTime $start, \DateTime $end, ?Department $department = null, ?User $user = null): int
    {
        $qb = $this->getEntityManager()->createQueryBuilder();
        ...
        if (null !== $department) {
            $qb
                ->andWhere($qb->expr()->eq('i.department', ':department'))
    ...
    public function getCounterForDay(\DateTimeInterface $date, ?Department $department = null, ?User $user = null): int
    {
        $date = \DateTime::createFromInterface($date);
        $start = (clone $date)->setTime(0, 0, 0);
        $end = (clone $date)->setTime(23, 59, 59);

        return $this->getCounterFor($start, $end, $department, $user);
    }

    public function getCounterForMonth(\DateTimeInterface $date, ?Department $department = null, ?User $user = null): int
    {
        $date = \DateTime::createFromInterface($date);
    ...
    public function getCounterForYear(\DateTimeInterface $date, ?Department $department = null, ?User $user = null): int
    {
        $date = \DateTime::createFromInterface($date);
    ...
    public function getCounterForDepartmentAllTime(?Department $department = null): int
    {
        if (null !== $department) {
            return $this->count(['department' => $department->getId()]);
        }
    ...
    private function getQueryBuilderForQuery(InvoiceArchiveQuery $query): QueryBuilder
    {
        ...
        if ($query->hasDepartments()) {
            $qb->andWhere($qb->expr()->in('i.department', ':department'));
        ...
        if ($query->hasSearchTerm()) {
            $qb->leftJoin('i.department', 'department');

            $configuration = new SearchConfiguration(
                ['i.comment', 'department.name', 'department.company'],
                InvoiceMeta::class,
                'invoice'
            );
    ...
    private function createInvoiceQuery(InvoiceArchiveQuery $invoiceArchiveQuery): Query
    {
        ...
        $query->setFetchMode(Invoice::class, 'department', ClassMetadata::FETCH_EAGER);
