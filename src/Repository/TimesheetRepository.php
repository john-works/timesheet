...
class TimesheetRepository extends EntityRepository
{
    ...
    public function getRawData(int $id): array
    {
        ...
        $qb
            ->select([
                ...
                't.hourlyRate',
                't.billable',
                'IDENTITY(p.department) as department',
    ...
    public function getTimesheetsForQuery(TimesheetQuery $query, bool $fullyHydrated = false): array
    {
        ...
        if ($fullyHydrated) {
            $query->addQueryHint(TimesheetQueryHint::DEPARTMENT_META_FIELDS);
    ...
    private function getQueryBuilderForQuery(TimesheetQuery $query): QueryBuilder
    {
        ...
        if ($query->hasDepartments()) {
            ...
            $qb->andWhere($qb->expr()->in('p.department', ':department'))
                ->setParameter('department', $query->getDepartments());
        }
        ...
        if ($requiresDepartment || $requiresProject || $requiresTeams) {
            $qb->leftJoin('t.project', 'p');
        }

        if ($requiresDepartment || $requiresTeams) {
            $qb->leftJoin('p.department', 'c');
        }
    ...
    public function getRecentActivityIds(User $user, ?\DateTimeInterface $startFrom = null, int $limit = 10): array
    {
        ...
        // do NOT join the department and do NOT check the department visibility, as this
        ...
        $qb->select('r, u, c')
            ...
            ->andWhere(
                ...
                $qb->expr()->orX(
                    ...
                    $qb->expr()->isNull('r.department')
                )
            )
            ->setParameter('user', $timesheet->getUser())
            ->setParameter('department', $timesheet->getProject()->getDepartment())
        ;
        $results = array_merge($results, $qb->getQuery()->getResult());
