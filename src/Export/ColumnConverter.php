...
final class ColumnConverter
{
    ...
    public function getColumns(TemplateInterface $template, TimesheetQuery $query): array
    {
        ...
        $departmentMeta = [];
        foreach ($this->findMetaColumns(new DepartmentMetaDisplayEvent($query->copyTo(new DepartmentQuery()), DepartmentMetaDisplayEvent::EXPORT)) as $metaField) {
            if ($metaField->getName() !== null) {
                $departmentMeta['department.meta.' . $metaField->getName()] = (new Column('department.meta.' . $metaField->getName(), $this->getFormatter('default')))
                    ->withHeader($metaField->getLabel())
                    ->withExtractor(function (ExportableItem $exportableItem) use ($metaField) {
                        return $exportableItem->getProject()?->getDepartment()?->getMetaField($metaField->getName())?->getValue();
                    });
            }
        }
        ...
        foreach ($template->getColumns($query) as $column) {
            if ($column === 'date') {
                ...
                $columns[$column] = (new Column('currency', $this->getFormatter('default')))->withExtractor(fn (ExportableItem $exportableItem) => $exportableItem->getProject()?->getDepartment()?->getCurrency())->withColumnWidth(ColumnWidth::SMALL);
            ...
            } elseif ($column === 'department.name') {
                $columns[$column] = (new Column('department', $this->getFormatter('default')))->withExtractor(fn (ExportableItem $exportableItem) => $exportableItem->getProject()?->getDepartment()?->getName())->withColumnWidth(ColumnWidth::MEDIUM);
            ...
            } elseif ($column === 'department.number') {
                $columns[$column] = (new Column('number', $this->getFormatter('default')))->withExtractor(fn (ExportableItem $exportableItem) => $exportableItem->getProject()?->getDepartment()?->getNumber());
            ...
            } elseif ($column === 'department.vat_id') {
                $columns[$column] = (new Column('vat_id', $this->getFormatter('default')))->withExtractor(fn (ExportableItem $exportableItem) => $exportableItem->getProject()?->getDepartment()?->getVatId());
            ...
            } elseif (str_starts_with($column, 'department.meta.') && \array_key_exists($column, $departmentMeta)) {
                $columns[$column] = $departmentMeta[$column];
