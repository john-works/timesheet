...
final class DepartmentMonthlyProjectsRepository
{
     ...
     * @internal
     */
    public function getGroupedByDepartmentProjectActivityUser(DateTime $begin, DateTime $end, array $users, ?Department $department): array
    {
        $stats = [];
        ...
        if ($department !== null) {
            ...
            $qb2->select('p.id')
                ...
                ->andWhere($qb2->expr()->eq('p.department', ':department'))
        ...
        $qb = $this->entityManager->createQueryBuilder();
        $qb
            ->select('p.id, p.name, c.id as department_id, c.name as department, c.currency')
            ->from(Project::class, 'p', 'p.id')
            ->leftJoin(Department::class, 'c', Join::WITH, 'c.id = p.department')
        ...
        foreach (array_keys($stats) as $pid) {
            ...
            $stats[$pid]['department'] = $projects[$pid]['department'];
            $stats[$pid]['department_id'] = $projects[$pid]['department_id'];
